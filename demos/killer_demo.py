...
def main():
    ...
    print(f"Calling send_email with:")
    ...
    try:
        bad_resp = vault.deliver(DeliverRequest(
            tool_call=ToolCall(
                name="send_email",
                args={
    ...
    try:
        cross_resp = vault.deliver(DeliverRequest(
            tool_call=ToolCall(
                name="send_email",
                args={
    ...
    try:
        forge_resp = vault.deliver(DeliverRequest(
            tool_call=ToolCall(
                name="send_email",
                args={
                    "to": "test@example.com",
...
valid: [[PII:PHONE:{phone_tok.ref}]]
    ...
    calls_before = len(executor.calls)
    try:
        parse_resp = vault.deliver(DeliverRequest(
            tool_call=ToolCall(
                name="send_email",
                args={
                    "to": "test@example.com",
        ...
        results.assert_contains(result, "missing_closer: [[PII:EMAIL:missing", "Malformed missing closer stays literal")
        results.assert_contains(result, "empty_ref: [[PII:EMAIL:]]", "Malformed empty ref stays literal")
        results.assert_contains(result, "missing_type: [[PII::ref]]", "Malformed missing type stays literal")
        ...
        print(f"  Blocked: {type(e).__name__} (fail-closed)")
        calls_after = len(executor.calls)
        results.assert_pass(calls_after == calls_before, "Tool NOT executed on parser error (fail-closed)")
    ...
    ticket_resp = vault.deliver(DeliverRequest(
        tool_call=ToolCall(
            name="create_ticket",
            args={
                "title": "Bug Report",
    ...
    result_json = json.dumps(ticket_resp.tool_result) if isinstance(ticket_resp.tool_result, dict) else str(ticket_resp.tool_result)
    ...
    error_msg = ""
    try:
        explode_resp = vault.deliver(DeliverRequest(
            tool_call=ToolCall(
                name="explode",
                args={
                    "message": f"Error for {email_text}"
                }
            ),
            vault_session=session_a
        ))
    ...
    print(f"  Initial tokenize: {tokenize_resp.audit_id if hasattr(tokenize_resp, 'audit_id') else 'N/A'}")
    ...
    chain_resp = vault.deliver(DeliverRequest(
        tool_call=ToolCall(
            name="send_email",
            args={
    ...
    results.assert_not_contains(str(chain_resp.tool_result), "alice@example.com", "Chained result scrubbed")
