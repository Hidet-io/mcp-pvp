...
from mcp_pvp.tokens import extract_text_tokens, TokenScanner, TEXT_TOKEN_PATTERN
...
def benchmark_scanner_vs_regex():
    ...
    if scanner_result.avg_time < regex_result.avg_time:
        ...
        print(f"  (Trade-off: O(n) worst-case vs potential regex backtracking)")
        speed_metric = -slowdown  # Negative indicates scanner is slower
    
    return {
        "regex": regex_result,
        "scanner": scanner_result,
        "speedup": speed_metric
    }
...
def benchmark_pathological_input():
    ...
    avg_ratio_diff = sum(abs(time_ratios[i] - size_ratios[i]) for i in range(len(time_ratios))) / len(time_ratios)
    print(f"  Average deviation from O(n): {avg_ratio_diff:.2f}")
    if avg_ratio_diff < 0.5:
        print(f"  ✅ Confirmed O(n) linear complexity")
...
def benchmark_recursive_scrubbing():
    ...
    return {
        "nested_dict": dict_results,
        "nested_list": list_results,
        "exception": exc_result
    }
...
def benchmark_large_sessions():
    ...
    tokenize_resp = vault.tokenize(TokenizeRequest(
        content=" ".join([f"user{i}@example.com" for i in range(10)])
    ))
    session_id = tokenize_resp.vault_session
    
    def deliver_func():
        vault.deliver(DeliverRequest(
...
def main():
    ...
    if scanner_speedup > 0:
        ...
        speedup_text = f"Scanner is {abs(scanner_speedup):.1f}x slower on simple input (but O(n) guaranteed)"
...
def generate_report(results: dict):
    ...
    if scanner_speedup > 0:
        ...
        scanner_summary = f"{abs(scanner_speedup):.1f}x slower on simple input (but O(n) guaranteed)"
...
"""
    ...
    for name, result in results["pathological_input"].items():
        if "brackets" in name:
            ...
            report += f"| {name} | {input_size:,} | {result.avg_time*1000:.4f} | {throughput:.2f} |\n"
