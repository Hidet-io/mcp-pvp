...
class DemoToolExecutor(ToolExecutor):
    ...
    def execute(self, tool_name: str, injected_args: dict) -> dict:
        ...
        if tool_name == "get_user_profile":
            ...
            return {
                "logs": [
                    {"timestamp": "2026-02-01T10:00:00", "user": "admin@system.com", "action": "login"},
                    {"timestamp": "2026-02-01T10:05:00", "user": "user@example.com", "action": "update"},
                    {"timestamp": "2026-02-01T10:10:00", "user": "bob@company.com", "action": "delete"}
                ],
                "total": 3
            }
...
def main():
    ...
    tokenize_req2 = TokenizeRequest(
        content="Admin access: bob@company.com"
    )
    ...
    token_ref = token_from_session1.pii_ref if hasattr(token_from_session1, 'pii_ref') else token_from_session1.ref
    try:
    ...
    print(f"✓ Executed tool: get_user_profile")
    ...
    for i, token in enumerate(deliver_resp.result_tokens[:3]):  # Show first 3
        ...
        token_ref = token.ref if hasattr(token, 'ref') else (token.pii_ref if hasattr(token, 'pii_ref') else str(token))
        try:
            pii = vault.store.get_pii(token_ref, session1)
            print(f"  {i+1}. {token_ref} -> {pii.pii_type} (session: {pii.vault_session == session1})")
        except Exception as e:
            print(f"  {i+1}. {token_ref} -> Token from result (type: {token.pii_type if hasattr(token, 'pii_type') else 'unknown'})")
    
    print(f"\n✓ Recursive scrubbing applied to nested result:")
    print(f"  Original result had nested dict with multiple emails")
    print(f"  Scrubbed result contains {deliver_resp.tool_result.count('[[PII:EMAIL:')} email tokens")
    ...
    pathological_tokenize = vault.tokenize(TokenizeRequest(
        content=f"Test: {pathological_input}"
    ))
    elapsed = time.perf_counter() - start
    
    print(f"✓ Processed pathological input (1000+ brackets)")
    print(f"  Time: {elapsed*1000:.2f}ms")
    print(f"  Tokens found: {len(pathological_tokenize.tokens)}")
    print(f"  Scanner handles false starts efficiently (O(n) complexity)")
    ...
    for deliver_event in deliver_events[-2:]:  # Show last 2
        ...
        if children:
            ...
            print(f"  No child events (no PII in result)")
    ...
    error_resp = vault.deliver(DeliverRequest(
        vault_session=session1,
        run=RunContext(run_id="demo_run", participant_id="demo_agent"),
        tool_call=ToolCall(name="error_prone_operation", args={}),
    ))
    
    print(f"✓ Tool raised exception with PII in traceback")
    print(f"✓ Exception was recursively scrubbed")
    print(f"  Result tokens from exception: {len(error_resp.result_tokens)}")
    print(f"  Scrubbed result preview:")
